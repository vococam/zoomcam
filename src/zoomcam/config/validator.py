...
class ConfigValidator:
    ...
    def __init__(self):
        ...
        self.rules = {
            ...
            'system.display.refresh_rate': lambda v: self._validate_range(v, 1, 120, int),
            'system.performance.max_cpu_usage': lambda v: self._validate_percentage(v),
            'system.performance.memory_limit': lambda v: self._validate_range(v, 64, 8192, int),
            ...
            'cameras.*.recording.quality': lambda v: self._validate_choice(v, ['low', 'medium', 'high']),
            'cameras.*.recording.reaction_time': lambda v: self._validate_range(v, 0.1, 10.0, float),
            'cameras.*.recording.max_duration': lambda v: self._validate_range(v, 10, 3600, int),

            # Motion detection validation
            'cameras.*.motion_detection.sensitivity': lambda v: self._validate_range(v, 0.0, 1.0, float),
            'cameras.*.motion_detection.min_area': lambda v: self._validate_range(v, 10, 10000, int),
            'cameras.*.motion_detection.max_zones': lambda v: self._validate_range(v, 1, 10, int),

            # Layout validation
            'layout.algorithm': lambda v: self._validate_choice(v, ['adaptive_grid', 'priority_based', 'equal_grid']),
            ...
            'recording.compression': lambda v: self._validate_choice(v, ['h264_fast', 'h264_medium', 'h264_slow']),

            # Git logging validation
            'logging.git.enabled': self._validate_boolean,
            'logging.git.commit_interval': lambda v: self._validate_range(v, 10, 3600, int),
            'logging.git.screenshot_interval': lambda v: self._validate_range(v, 1, 300, int),
            'logging.git.max_history_days': lambda v: self._validate_range(v, 1, 365, int),
        }
    ...
    def _validate_cross_fields(self, config: Dict[str, Any]):
        ...
        try:
            ...
            system_res = config.get('system', {}).get('display', {}).get('target_resolution')
            if system_res:
                ...
                for camera_id, camera_config in config.get('cameras', {}).items():
                    ...
                    if camera_res != 'auto':
                        try:
                            ...
                            if cam_width > system_width or cam_height > system_height:
                                self.warnings.append(ValidationError(
                                    field=f"cameras.{camera_id}.resolution",
                                    message=f"Camera resolution {camera_res} exceeds system resolution {system_res}",
                                    value=camera_res,
                                    suggestion=f"Reduce camera resolution or increase system resolution",
                                    severity="warning"
                                ))
                        except ValueError:
                            pass  # Invalid resolution format already caught by field validation

            # Check for excessive recording settings
            recording_enabled_count = sum(
                1 for c in config.get('cameras', {}).values() 
                if c.get('recording', {}).get('enabled', False)
            )
                ...
                if source:
                    if source in used_sources:
                        self.errors.append(ValidationError(
                            ...
                            value=source,
                            suggestion=f"Each camera must have a unique source"
                        ))
                    used_sources.add(source)

            # Check for overlapping motion detection zones
            # This is a simplified example - actual implementation would need more context
            
            # Check for invalid combinations
            if (config.get('system', {}).get('performance', {}).get('max_cpu_usage', 80) > 90 and
                any(c.get('recording', {}).get('enabled') for c in config.get('cameras', {}).values())):
                self.warnings.append(ValidationError(
                    field="system.performance.max_cpu_usage",
                    message="High CPU usage with recording enabled may cause performance issues",
                    value=config['system']['performance']['max_cpu_usage'],
                    suggestion="Reduce max CPU usage or disable some recording streams",
                    severity="warning"
                ))

            # Check performance implications
            active_cameras = len([c for c in config.get('cameras', {}).values() if c.get('enabled', True)])
    ...
    def _parse_resolution(self, resolution: str) -> Tuple[int, int]:
        ...
        try:
            ...
            raise ValueError(f"Invalid resolution format: {resolution}. Expected WxH (e.g., 1920x1080)")
    ...
    def _validate_range(self, value: Any, min_val: float, max_val: float, value_type: type) -> Optional[str]:
        ...
        try:
            ...
            return f"Value must be a {value_type.__name__} between {min_val} and {max_val}"
            ...
            if system_res:
                ...
                for camera_id, camera_config in config.get('cameras', {}).items():
                    ...
                    if camera_res != 'auto':
                        try:
                            cam_width, cam_height = self._parse_resolution(camera_res)
                            if cam_width > system_width * 2 or cam_height > system_height * 2:
                                self.warnings.append(ValidationError(
                                    field=f"cameras.{camera_id}.resolution",
