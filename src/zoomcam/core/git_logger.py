...
class GitLogger:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logging.info(
            f"Git Logger initialized: {'enabled' if self.enabled else 'disabled'}"
        )
...
"""
        ...
        if GIT_AVAILABLE:
            self.repo.index.add(
                [str(readme_path)]
                + [
                    str(self.repo_path / d / ".gitkeep")
                    for d in ["events", "configs", "screenshots", "performance"]
                ]
            )
    ...
    async def _process_single_event(self, event: LogEvent) -> None:
        ...
        try:
            ...
            event_dir = (
                self.repo_path / "events" / f"{timestamp_str}_{event.event_type}"
            )
            ...
            if event.config_snapshot:
                ...
                with open(config_file, "w") as f:
                    yaml.dump(
                        event.config_snapshot, f, default_flow_style=False, indent=2
                    )
    ...
    async def _commit_files(self, files: List[str], event: LogEvent) -> None:
        ...
        try:
            ...
            if (
                ...
                and (current_time - self.last_commit_time).total_seconds()
            ...
            if old_dirs:
                if GIT_AVAILABLE and self.repo:
                    ...
                    self.repo.index.commit(
                        f"Cleanup: Removed {len(old_dirs)} old event directories"
                    )
                else:
                    subprocess.run(
                        ["git", "add", "events/"], cwd=self.repo_path, check=True
                    )
                    subprocess.run(
                        [
                            "git",
    ...
    async def log_layout_change(
        self, layout_data: Dict[str, Any], screenshot: Optional[np.ndarray] = None
    ) -> None:
    ...
    async def log_config_change(
        self, config_data: Dict[str, Any], config_snapshot: Dict[str, Any]
    ) -> None:
    ...
    async def log_camera_event(
        ...
        await self.log_event(
            event_type=event_type, data=data, summary=summary, camera_id=camera_id
        )
    ...
    async def log_performance_alert(self, performance_data: Dict[str, Any]) -> None:
        ...
        await self.log_event(
            event_type="performance_alert", data=performance_data, summary=summary
        )

    async def get_timeline_events(
        self, limit: int = 100, filter_type: Optional[str] = None
    ) -> List[Dict[str, Any]]:
            ...
            for event_dir in event_dirs:
                ...
                try:
                    ...
                    if screenshot_file.exists():
                        event_data[
                            "screenshot"
                        ] = f"/screenshots/{screenshot_file.name}"
        ...
        try:
            if GIT_AVAILABLE and self.repo:
                ...
                for commit in commits:
                    if event_name in commit.message or any(
                        event_name in f for f in commit.stats.files
                    ):
                        return {
                            "sha": commit.hexsha[:8],
    ...
    async def shutdown(self) -> None:
        ...
        if remaining_events > 0:
            logging.info(
                f"Processed {remaining_events} remaining events during shutdown"
            )
