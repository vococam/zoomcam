    ...
    cameras.forEach((camera, id) => {
      css += `
                .camera-${id} {
                    ...
                    border: ${
                      camera.active ? "3px solid #00ff00" : "1px solid #666"
                    };
...
class RealTimeMonitor {
  ...
  updateAdminPanel(data) {
    ...
    data.cameras.forEach((camera, id) => {
      document.getElementById(`source-res-${id}`).textContent =
        camera.source_resolution;
      document.getElementById(`target-res-${id}`).textContent =
        camera.target_resolution;
      document.getElementById(`scale-ratio-${id}`).textContent =
        `${camera.scale_ratio}x`;
    });
