...
class SetupWizard {
    ...
    cameras.forEach((camera, index) => {
      ...
      html += `
                <div class="camera-item">
                    ...
                    <button class="btn btn-secondary" onclick="wizard.testCamera('${
                      camera.source
                    }', ${index})">
                        Test Camera
                    </button>
  ...
  async testCamera(source, index) {
    ...
    try {
      ...
      if (data.success) {
        ...
        resultSpan.innerHTML = ` ❌ <span style="color: red;">Failed: ${
  ...
  async configureSystem() {
    ...
    this.detectedCameras.forEach((camera, index) => {
      ...
      if (enabledCheckbox && enabledCheckbox.checked) {
        cameras.push({
          ...
          max_fragments: parseInt(
            document.getElementById(`fragments-${index}`).value,
          ),
          recording_enabled: document.getElementById(`recording-${index}`)
            .checked,
          motion_sensitivity:
            parseFloat(document.getElementById(`sensitivity-${index}`).value) /
            100,
        });
      }
    });
    ...
    try {
      ...
      this.showError(
        "Network error during camera configuration: " + error.message,
      );
    }
  }
  ...
  async testMultipleRTSP(urls, timeout = 10) {
    ...
    for (let i = 0; i < urls.length; i += concurrency) {
      ...
      const batchPromises = batch.map((url) =>
        this.testSingleRTSP(url, timeout),
      );
